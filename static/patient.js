...
function showSlots() {
    $('#info_area').empty();
    const query = '/schedule?type=patient';
    ...
    fetch(query)
        ...
        .then(json => {
            for (let id in json.times) {
                ...
                const postQuery = '/schedule?type=patient&inputSlot=' + slot;
                card.click(function () {
...
function showMySlots() {
    $('#info_area').empty();
    const query = '/schedule?type=Patient&Personal=true';
    ...
    fetch(query)
        ...
        .then(json => {
            for (let id in json.times) {
                ...
                const postQuery = '/schedule?action=Remove&type=patient&inputSlot=' + slot;
                card.click(function () {
                    $.post(postQuery);
                });
            }
